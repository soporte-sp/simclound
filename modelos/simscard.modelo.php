...
class ModeloSimscards
{
	...
	static public function mdlAsignarSimcards($tabla, $datos)
	{
		...
		$stmt = Conexion::conectar()->prepare("UPDATE $tabla SET usuario = :usuario, simcard = :simcard, addcoodininve = :addcoodininve  WHERE id = :id");

		$stmt->bindParam(":id", $datos["id"], PDO::PARAM_STR);
		$stmt->bindParam(":usuario", $datos["usuario"], PDO::PARAM_STR);
		$stmt->bindParam(":simcard", $datos["simcard"], PDO::PARAM_STR);
		$stmt->bindParam(":addcoodininve", $datos["addcoodininve"], PDO::PARAM_STR);
	...
	{
		//																					simcards     valor  =    1        simcard = 45246244 este dato es de la simcard
	...
	=============================================*/
	static public function mdlMostrarSinscardAgencias($tabla, $item, $valor, $select, $valor1, $select1, $valor2, $select2)
	{
		...
		if ($item != null) {
			...
			$stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE $select = :$select OR $select1 = :$select1 OR $select2 = :$select2");

			$stmt->bindParam(":" . $select, $valor, PDO::PARAM_STR);
			$stmt->bindParam(":" . $select1, $valor1, PDO::PARAM_STR);
			$stmt->bindParam(":" . $select2, $valor2, PDO::PARAM_STR);
