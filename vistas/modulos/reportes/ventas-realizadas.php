...
if ($_SESSION["perfil"] == "Administrador") {
    ...
    foreach ($respuesta as $key => $value) {
        ...
        if (is_array($value) && $value["fechaventa"]) {
            ...
            foreach ($arrayVentas as $key => $value1) {
                $sumaPagosMes[$key] += $value1;
            }
        }
    }
    ...
    foreach ($ventas as $key => $valueVentas) {
        foreach ($usuarios as $key => $valueUsuarios) {
            ...
            if ($valueUsuarios["id"] == $valueVentas["vendedor"]) {
                ...
                if ($valueUsuarios["nombre"]) array_push($arrayVendedores, $valueUsuarios["nombre"]);
                #Capturamos las nombres y los valores netos en un mismo array
                if ($valueVentas["valor"]) $arraylistaVendedores = array($valueUsuarios["nombre"] => $valueVentas["valor"]);
                #Sumamos los netos de cada vendedor
                //var_dump($arraylistaVendedores);
    ...
    $usuarios = ControladorUsuarios::ctrMostrarUsuarios($item, $valor, $perfil, $valor1, $perfil1);
    $ventas = ControladorVentas::ctrMostrarVentas($item, $perfil25, $item1, $valor3,$_SESSION['perfil']);
    ...
    // $usuarios1 = ControladorClientes::ctrMostrarClientes($item1,$valor1,$perfil);
    $ventas1 = ControladorVentas::ctrMostrarVentas($item, $perfil15, $item16, $valor30,$_SESSION['perfil']);
    ...
    <div class="col-lg-2">
        <select style="width: 10%;" class="form-control m-b chosen-select" id="clientesValor" data-placeholder="Escoge el usuario" tabindex="2">
            ...
            if ($_SESSION["perfil"] == "Administrador") {
                ...
                $clientes = ControladorVentas::ctrMostrarVentas($item, $valor, $item1, $valor3,$_SESSION['perfil']);
            } else if ($_SESSION["perfil"] == "Agencias") {
                $item = null;
                $valor = null;
                $item1 = "agregopadre";
